...
export function UserSidebar({ isSafetyLocked = false }: { isSafetyLocked?: boolean }) {
    ...
    >
      ...
      <nav className="space-y-1">
        {userNavItems
          ...
          .map((item) => {
            ...
            const locked = isSafetyLocked;
            const className = cn(
              "flex min-h-12 items-center justify-center gap-3 rounded-[14px] px-3 py-3 text-[#E7E4D8] transition-colors focus-visible:ring-white/30 lg:justify-start lg:px-4",
              locked
                ? "cursor-not-allowed opacity-35"
                : "hover:bg-white/10",
              active && !locked && "bg-white/[0.12] text-white shadow-[inset_3px_0_0_rgba(192,97,58,0.95)]",
            );
            ...
            if (locked) {
              return (
                <button
                  ...
                  className={className}
                  key={item.href}
                  title="Complete the safety gate before using the app."
                  type="button"
                >
      ...
      <div className="space-y-2">
        ...
        {isSafetyLocked ? (
          <button
            aria-disabled="true"
            className="flex min-h-14 w-full cursor-not-allowed items-center justify-center gap-3 rounded-[16px] bg-white/8 px-3 py-4 text-white/45 lg:justify-start lg:px-4"
            title="Complete the safety gate before starting a daily check-in."
            type="button"
          >
...
export function UserBottomNavigation({ isSafetyLocked = false }: { isSafetyLocked?: boolean }) {
  const pathname = usePathname();
    ...
    >
      <div className="grid h-[76px] w-full grid-cols-5 items-center rounded-[28px] border border-sedona-creamLine bg-white/95 px-3 shadow-nav backdrop-blur">
        {userNavItems.map((item) => {
          ...
          const locked = isSafetyLocked;
          const className = cn(
            "group relative flex min-h-14 min-w-0 touch-manipulation flex-col items-center justify-center gap-1 rounded-[18px] text-center transition-[color,transform,background-color] duration-200 focus-visible:ring-2 focus-visible:ring-sedona-copper focus-visible:ring-offset-2 focus-visible:ring-offset-white",
            item.featured
              ? "-mt-7 h-[68px] w-[68px] justify-center rounded-full bg-sedona-copper text-white shadow-[0_14px_28px_-14px_rgba(176,79,36,0.85)]"
              : "text-[#A99C86]",
            locked
              ? "cursor-not-allowed opacity-35"
              : "active:scale-[0.96] hover:bg-sedona-sand/70 hover:text-sedona-pineSoft",
            active && !item.featured && !locked && "bg-sedona-sand text-sedona-clay",
            active && item.featured && !locked && "bg-sedona-clay text-white shadow-[0_16px_32px_-14px_rgba(143,62,27,0.8)]",
          );

          const content = (
            <>
              {!item.featured && active && !locked ? (
              ...
              <Icon
                ...
                strokeWidth={active && !locked ? 2.2 : 1.9}
              />
              <span
                className={cn(
                  "max-w-full truncate font-semibold leading-none",
                  item.featured ? "text-xs" : "text-[11px]",
                  active && !item.featured && !locked && "text-sedona-clay",
                )}
              >
                {item.label}
              </span>
          ...
          if (locked) {
            return (
              <button
                aria-disabled="true"
                aria-label={item.featured ? "Today check-in unavailable" : `${item.label} unavailable`}
                className={className}
                key={item.href}
                title="Complete the safety gate before using the app."
                type="button"
